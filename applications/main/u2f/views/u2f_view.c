...
static void u2f_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->display_msg == U2fMsgNotConnected) {
        canvas_draw_icon(canvas, 22, 15, D_ASSETS()->I_Connect_me_62x31);
        if(D_ASSETS()->is_nsfw) {
        ...
        canvas_draw_icon(canvas, 22, 15, D_ASSETS()->I_Connected_62x31);
        ...
        if(D_ASSETS()->is_nsfw) {
            ...
            canvas_draw_icon(canvas, 22, 15, D_ASSETS()->I_Auth_62x31);
            canvas_draw_str_aligned(
        ...
        canvas_draw_icon(canvas, 22, 15, D_ASSETS()->I_Error_62x31);
        if(D_ASSETS()->is_nsfw) {
