...
static bool xvirus_app_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
...
bool xvirus_app_apply(XvirusApp* app) {
    Storage* storage = furi_record_open(RECORD_STORAGE);

    if(app->save_mainmenu_apps) {
        Stream* stream = file_stream_alloc(storage);
        if(file_stream_open(stream, XVIRUS_APPS_PATH, FSAM_READ_WRITE, FSOM_CREATE_ALWAYS)) {
            CharList_it_t it;
    ...
    if(app->require_reboot) {
        ...
        view_dispatcher_switch_to_view(app->view_dispatcher, XvirusAppViewPopup);
        return true;
    }
...
static bool xvirus_app_back_event_callback(void* context) {
    furi_assert(context);
    XvirusApp* app = context;

    if(!scene_manager_has_previous_scene(app->scene_manager, XvirusAppSceneStart)) {
        if(xvirus_app_apply(app)) {
            return true;
        }
    }
...
XvirusApp* xvirus_app_alloc() {
    ...
    app->scene_manager = scene_manager_alloc(&xvirus_app_scene_handlers, app);
    view_dispatcher_enable_queue(app->view_dispatcher);
    view_dispatcher_set_event_callback_context(app->view_dispatcher, app);

    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, xvirus_app_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, xvirus_app_back_event_callback);
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, XvirusAppViewTextInput, text_input_get_view(app->text_input));

    app->popup = popup_alloc();
    view_dispatcher_add_view(app->view_dispatcher, XvirusAppViewPopup, popup_get_view(app->popup));
        ...
        while(storage_dir_read(folder, &info, name, MAX_PACK_NAME_LEN)) {
            if(info.flags & FSF_DIRECTORY) {
                ...
                if(app->asset_pack_index != 0) {
                    ...
                    if(strcmp(copy, xvirus_settings->asset_pack) == 0)
    ...
    if(file_stream_open(stream, XVIRUS_APPS_PATH, FSAM_READ, FSOM_OPEN_EXISTING)) {
...
void xvirus_app_free(XvirusApp* app) {
    furi_assert(app);

    // Gui modules
    view_dispatcher_remove_view(app->view_dispatcher, XvirusAppViewVarItemList);
    variable_item_list_free(app->var_item_list);
    view_dispatcher_remove_view(app->view_dispatcher, XvirusAppViewTextInput);
    text_input_free(app->text_input);
    view_dispatcher_remove_view(app->view_dispatcher, XvirusAppViewPopup);
...
extern int32_t xvirus_app(void* p) {
    UNUSED(p);
    XvirusApp* app = xvirus_app_alloc();
    snprintf(app->fap_name, 32, "%s.fap", furi_thread_get_appid(furi_thread_get_current_id()));
    scene_manager_next_scene(app->scene_manager, XvirusAppSceneStart);
