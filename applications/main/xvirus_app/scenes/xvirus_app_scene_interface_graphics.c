...
void xvirus_app_scene_interface_graphics_var_item_list_callback(void* context, uint32_t index) {
    XvirusApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void xvirus_app_scene_interface_graphics_asset_pack_changed(VariableItem* item) {
...
const int32_t anim_speed_values[COUNT_OF(anim_speed_names)] =
    {25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300};
static void xvirus_app_scene_interface_graphics_anim_speed_changed(VariableItem* item) {
    XvirusApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void xvirus_app_scene_interface_graphics_cycle_anims_changed(VariableItem* item) {
    XvirusApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void xvirus_app_scene_interface_graphics_unlock_anims_changed(VariableItem* item) {
    XvirusApp* app = variable_item_get_context(item);
...
static void xvirus_app_scene_interface_graphics_fallback_anim_changed(VariableItem* item) {
    XvirusApp* app = variable_item_get_context(item);
    bool value = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, value ? "ON" : "OFF");
    XVIRUS_SETTINGS()->fallback_anim = value;
    app->save_settings = true;
}

void xvirus_app_scene_interface_graphics_on_enter(void* context) {
    XvirusApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        xvirus_app_scene_interface_graphics_asset_pack_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        xvirus_app_scene_interface_graphics_anim_speed_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        xvirus_app_scene_interface_graphics_cycle_anims_changed,
        app);
    value_index = value_index_int32(
    ...
    item = variable_item_list_add(
        ...
        "Unlock Anims",
        2,
        xvirus_app_scene_interface_graphics_unlock_anims_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        "Fallback Anim",
        2,
        xvirus_app_scene_interface_graphics_fallback_anim_changed,
        app);
    variable_item_set_current_value_index(item, xvirus_settings->fallback_anim);
    variable_item_set_current_value_text(item, xvirus_settings->fallback_anim ? "ON" : "OFF");

    variable_item_list_set_enter_callback(
        var_item_list, xvirus_app_scene_interface_graphics_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, XvirusAppSceneInterfaceGraphics));

    view_dispatcher_switch_to_view(app->view_dispatcher, XvirusAppViewVarItemList);
}

bool xvirus_app_scene_interface_graphics_on_event(void* context, SceneManagerEvent event) {
    XvirusApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, XvirusAppSceneInterfaceGraphics, event.event);
        consumed = true;
        switch(event.event) {
        default:
            break;
...
void xvirus_app_scene_interface_graphics_on_exit(void* context) {
    XvirusApp* app = context;
