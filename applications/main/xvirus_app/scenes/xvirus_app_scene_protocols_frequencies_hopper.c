...
void xvirus_app_scene_protocols_frequencies_hopper_var_item_list_callback(
    void* context,
    uint32_t index) {
...
static void xvirus_app_scene_protocols_frequencies_hopper_frequency_changed(VariableItem* item) {
...
void xvirus_app_scene_protocols_frequencies_hopper_on_enter(void* context) {
    XvirusApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        xvirus_app_scene_protocols_frequencies_hopper_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, xvirus_app_scene_protocols_frequencies_hopper_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(
            app->scene_manager, XvirusAppSceneProtocolsFrequenciesHopper));

    view_dispatcher_switch_to_view(app->view_dispatcher, XvirusAppViewVarItemList);
}

bool xvirus_app_scene_protocols_frequencies_hopper_on_event(void* context, SceneManagerEvent event) {
    XvirusApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, XvirusAppSceneProtocolsFrequenciesHopper, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddHopperFreq:
            scene_manager_set_scene_state(
                app->scene_manager, XvirusAppSceneProtocolsFrequenciesAdd, true);
...
void xvirus_app_scene_protocols_frequencies_hopper_on_exit(void* context) {
    XvirusApp* app = context;
