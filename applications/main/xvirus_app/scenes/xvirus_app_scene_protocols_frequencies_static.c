...
void xvirus_app_scene_protocols_frequencies_static_var_item_list_callback(
    void* context,
    uint32_t index) {
...
static void xvirus_app_scene_protocols_frequencies_static_frequency_changed(VariableItem* item) {
...
void xvirus_app_scene_protocols_frequencies_static_on_enter(void* context) {
    XvirusApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        xvirus_app_scene_protocols_frequencies_static_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, xvirus_app_scene_protocols_frequencies_static_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(
            app->scene_manager, XvirusAppSceneProtocolsFrequenciesStatic));

    view_dispatcher_switch_to_view(app->view_dispatcher, XvirusAppViewVarItemList);
}

bool xvirus_app_scene_protocols_frequencies_static_on_event(void* context, SceneManagerEvent event) {
    XvirusApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, XvirusAppSceneProtocolsFrequenciesStatic, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddStaticFreq:
            scene_manager_set_scene_state(
                app->scene_manager, XvirusAppSceneProtocolsFrequenciesAdd, false);
            scene_manager_next_scene(app->scene_manager, XvirusAppSceneProtocolsFrequenciesAdd);
            break;
        default:
            break;
        }
...
void xvirus_app_scene_protocols_frequencies_static_on_exit(void* context) {
    XvirusApp* app = context;
