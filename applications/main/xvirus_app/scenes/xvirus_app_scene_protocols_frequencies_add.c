...
static void xvirus_app_scene_protocols_frequencies_add_text_input_callback(void* context) {
    ...
    bool is_hopper =
        scene_manager_get_scene_state(app->scene_manager, XvirusAppSceneProtocolsFrequenciesAdd);
    if(is_hopper) {
...
void xvirus_app_scene_protocols_frequencies_add_on_enter(void* context) {
    XvirusApp* app = context;
    TextInput* text_input = app->text_input;

    text_input_set_header_text(text_input, "Format: 12356");

    strlcpy(app->subghz_freq_buffer, "", XVIRUS_SUBGHZ_FREQ_BUFFER_SIZE);

    text_input_set_result_callback(
        text_input,
        xvirus_app_scene_protocols_frequencies_add_text_input_callback,
        app,
        app->subghz_freq_buffer,
        XVIRUS_SUBGHZ_FREQ_BUFFER_SIZE,
        true);
...
bool xvirus_app_scene_protocols_frequencies_add_on_event(void* context, SceneManagerEvent event) {
    XvirusApp* app = context;
    bool consumed = false;
...
void xvirus_app_scene_protocols_frequencies_add_on_exit(void* context) {
    XvirusApp* app = context;
