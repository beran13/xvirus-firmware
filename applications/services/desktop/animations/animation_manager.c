    ...
    if(animation_manager->state == AnimationManagerStateIdle) {
        ...
        if(!blocked) {
            ...
            bool valid = animation_manager_is_valid_idle_animation(
                manifest_info, &stats, XVIRUS_SETTINGS()->unlock_anims);
...
static void animation_manager_start_new_idle(AnimationManager* animation_manager) {
    ...
    int32_t duration = (xvirus_settings->cycle_anims == 0) ? (bubble_animation->duration) :
                                                             (xvirus_settings->cycle_anims);
    furi_timer_start(
...
static StorageAnimation*
    ...
    bool fallback = XVIRUS_SETTINGS()->fallback_anim;
    ...
    bool unlock = XVIRUS_SETTINGS()->unlock_anims;
            ...
            if(restore_animation) {
                ...
                if(valid) {
                    ...
                    if(animation_manager->freezed_animation_time_left) {
                        ...
                        XvirusSettings* xvirus_settings = XVIRUS_SETTINGS();
                        int32_t duration = (xvirus_settings->cycle_anims == 0) ?
...
static void animation_manager_switch_to_one_shot_view(AnimationManager* animation_manager) {
    ...
    one_shot_view_start_animation(
        animation_manager->one_shot_view, XVIRUS_ASSETS()->A_Levelup_128x64);
}
