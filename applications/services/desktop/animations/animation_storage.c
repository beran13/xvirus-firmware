...
void animation_handler_select_manifest(bool force_stock) {
    ...
    bool use_asset_pack = !force_stock && xvirus_settings->asset_pack[0] != '\0';
    if(use_asset_pack) {
        furi_string_printf(anim_dir, "%s/%s/Anims", PACKS_DIR, xvirus_settings->asset_pack);
...
static BubbleAnimation* animation_storage_load_animation(const char* name) {
    ...
    do {
        ...
        uint16_t anim_speed = XVIRUS_SETTINGS()->anim_speed;
