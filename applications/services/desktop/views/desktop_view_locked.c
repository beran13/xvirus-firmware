...
void desktop_view_locked_draw_lockscreen(Canvas* canvas, void* m) {
    ...
    canvas_draw_icon(canvas, 0, 0 + y, XVIRUS_ASSETS()->I_Lockscreen);
    if(xvirus_settings->lockscreen_time) {
    ...
    if(xvirus_settings->lockscreen_date) {
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(canvas, 0, 48 + y + 16 * !xvirus_settings->lockscreen_time, date_str);
    }
    if(model->view_state == DesktopViewLockedStateLockedHintShown &&
       xvirus_settings->lockscreen_prompt) {
        canvas_set_font(canvas, FontSecondary);
        if(model->pin_locked) {
            elements_bubble_str(
