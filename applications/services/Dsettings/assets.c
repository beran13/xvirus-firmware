...
DAssets* d_assets = NULL;

void anim(const Icon** replace, const char* name, FuriString* path, File* file) {
    do {
        furi_string_printf(path, ICONS_FMT "/meta", D_SETTINGS()->asset_pack, name);
        ...
        const char* pack = D_SETTINGS()->asset_pack;
...
void icon(const Icon** replace, const char* name, FuriString* path, File* file) {
    furi_string_printf(path, ICONS_FMT ".bmx", D_SETTINGS()->asset_pack, name);
    if(storage_file_open(file, furi_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING)) {
...
void swap(DAssets* x, FuriString* p, File* f) {
...
void D_ASSETS_LOAD() {
    if(d_assets != NULL) return;

    d_assets = malloc(sizeof(DAssets));
    Dsettings* d_settings = D_SETTINGS();

    d_assets->A_Levelup_128x64 = &A_Levelup_128x64;
    d_assets->I_BLE_Pairing_128x64 = &I_BLE_Pairing_128x64;
    d_assets->I_DolphinCommon_56x48 = &I_DolphinCommon_56x48;
    d_assets->I_DolphinMafia_115x62 = &I_DolphinMafia_115x62;
    d_assets->I_DolphinNice_96x59 = &I_DolphinNice_96x59;
    d_assets->I_DolphinWait_61x59 = &I_DolphinWait_61x59;
    d_assets->I_iButtonDolphinVerySuccess_108x52 = &I_iButtonDolphinVerySuccess_108x52;
    d_assets->I_DolphinReadingSuccess_59x63 = &I_DolphinReadingSuccess_59x63;
    d_assets->I_NFC_dolphin_emulation_47x61 = &I_NFC_dolphin_emulation_47x61;
    d_assets->I_passport_bad = &I_passport_bad;
    d_assets->I_passport_DB = &I_passport_DB;
    d_assets->I_passport_happy = &I_passport_happy;
    d_assets->I_passport_okay = &I_passport_okay;
    d_assets->I_RFIDDolphinReceive_97x61 = &I_RFIDDolphinReceive_97x61;
    d_assets->I_RFIDDolphinSend_97x61 = &I_RFIDDolphinSend_97x61;
    d_assets->I_RFIDDolphinSuccess_108x57 = &I_RFIDDolphinSuccess_108x57;
    d_assets->I_Cry_dolph_55x52 = &I_Cry_dolph_55x52;
    d_assets->I_Scanning_123x52 = &I_Scanning_123x52;
    d_assets->I_Auth_62x31 = &I_Auth_62x31;
    d_assets->I_Connect_me_62x31 = &I_Connect_me_62x31;
    d_assets->I_Connected_62x31 = &I_Connected_62x31;
    d_assets->I_Error_62x31 = &I_Error_62x31;

    if(d_settings->asset_pack[0] == '\0') return;
    d_assets->is_nsfw = strncmp(d_settings->asset_pack, "NSFW", strlen("NSFW")) == 0;
    FileInfo info;
    FuriString* path = furi_string_alloc();
    furi_string_printf(path, PACKS_DIR "/%s", d_settings->asset_pack);
    Storage* storage = furi_record_open(RECORD_STORAGE);
    if(storage_common_stat(storage, furi_string_get_cstr(path), &info) == FSE_OK &&
       info.flags & FSF_DIRECTORY) {
        File* file = storage_file_alloc(storage);
        swap(d_assets, path, file);
