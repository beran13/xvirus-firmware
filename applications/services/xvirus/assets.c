...
#define TAG "XvirusAssets"

#define ICONS_FMT PACKS_DIR "/%s/Icons/%s"

XvirusAssets* xvirus_assets = NULL;

void anim(const Icon** replace, const char* name, FuriString* path, File* file) {
    do {
        furi_string_printf(path, ICONS_FMT "/meta", XVIRUS_SETTINGS()->asset_pack, name);
        ...
        const char* pack = XVIRUS_SETTINGS()->asset_pack;
...
void icon(const Icon** replace, const char* name, FuriString* path, File* file) {
    furi_string_printf(path, ICONS_FMT ".bmx", XVIRUS_SETTINGS()->asset_pack, name);
    if(storage_file_open(file, furi_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING)) {
...
void swap(XvirusAssets* x, FuriString* p, File* f) {
    ...
    icon(&x->I_passport_bad, "Passport/passport_bad", p, f);
    icon(&x->I_passport_DB, "Passport/passport_DB", p, f);
    icon(&x->I_passport_happy, "Passport/passport_happy", p, f);
    icon(&x->I_passport_okay, "Passport/passport_okay", p, f);
...
void XVIRUS_ASSETS_LOAD() {
    if(xvirus_assets != NULL) return;

    xvirus_assets = malloc(sizeof(XvirusAssets));

    xvirus_assets->A_Levelup_128x64 = &A_Levelup_128x64;
    xvirus_assets->I_BLE_Pairing_128x64 = &I_BLE_Pairing_128x64;
    xvirus_assets->I_DolphinCommon_56x48 = &I_DolphinCommon_56x48;
    xvirus_assets->I_DolphinMafia_115x62 = &I_DolphinMafia_115x62;
    xvirus_assets->I_DolphinNice_96x59 = &I_DolphinNice_96x59;
    xvirus_assets->I_DolphinWait_61x59 = &I_DolphinWait_61x59;
    xvirus_assets->I_iButtonDolphinVerySuccess_108x52 = &I_iButtonDolphinVerySuccess_108x52;
    xvirus_assets->I_DolphinReadingSuccess_59x63 = &I_DolphinReadingSuccess_59x63;
    xvirus_assets->I_NFC_dolphin_emulation_47x61 = &I_NFC_dolphin_emulation_47x61;
    xvirus_assets->I_passport_bad = &I_passport_bad;
    xvirus_assets->I_passport_DB = &I_passport_DB;
    xvirus_assets->I_passport_happy = &I_passport_happy;
    xvirus_assets->I_passport_okay = &I_passport_okay;
    xvirus_assets->I_RFIDDolphinReceive_97x61 = &I_RFIDDolphinReceive_97x61;
    xvirus_assets->I_RFIDDolphinSend_97x61 = &I_RFIDDolphinSend_97x61;
    xvirus_assets->I_RFIDDolphinSuccess_108x57 = &I_RFIDDolphinSuccess_108x57;
    xvirus_assets->I_Cry_dolph_55x52 = &I_Cry_dolph_55x52;
    xvirus_assets->I_Fishing_123x52 = &I_Fishing_123x52;
    xvirus_assets->I_Scanning_123x52 = &I_Scanning_123x52;
    xvirus_assets->I_Auth_62x31 = &I_Auth_62x31;
    xvirus_assets->I_Connect_me_62x31 = &I_Connect_me_62x31;
    xvirus_assets->I_Connected_62x31 = &I_Connected_62x31;
    xvirus_assets->I_Error_62x31 = &I_Error_62x31;
    ...
    if(xvirus_settings->asset_pack[0] == '\0') return;
    xvirus_assets->is_nsfw = strncmp(xvirus_settings->asset_pack, "NSFW", strlen("NSFW")) == 0;
...
XvirusAssets* XVIRUS_ASSETS() {
    if(xvirus_assets == NULL) {
        XVIRUS_ASSETS_LOAD();
    }
    return xvirus_assets;
}
