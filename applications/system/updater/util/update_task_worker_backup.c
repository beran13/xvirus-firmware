...
#define DM0FIRSTBOOT_FLAG_PATH CFG_PATH("dm0firstboot.flag")
...
static bool update_task_post_update(UpdateTask* update_task) {
    ...
    do {
        ...
        if(update_task->state.groups & UpdateTaskStageGroupSplashscreen) {
            ...
            if(storage_common_stat(update_task->storage, DM0FIRSTBOOT_FLAG_PATH, NULL) ==
               FSE_NOT_EXIST) {
                File* file = storage_file_alloc(update_task->storage);
                if(storage_file_open(
                       file, DM0FIRSTBOOT_FLAG_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
                    storage_file_close(file);
                }
                storage_file_free(file);
                path_append(tmp_path, "dm0firstboot.bin");
